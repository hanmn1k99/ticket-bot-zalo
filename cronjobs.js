...
function setupCronJobs(sendZaloMessage) {
  ...
  cron.schedule('0 10 28-31 * *', async () => {
    ...
    if (tomorrow.getDate() === 1) {
      ...
      if (adminId) {
        await sendZaloMessage(adminId, "Hôm nay là ngày cuối tháng. Hãy gõ /report để xuất báo cáo dữ liệu yêu cầu trong tháng trước khi hệ thống dọn dẹp vào ngày mai nhé!");
      }
    }
  });
  ...
  cron.schedule('1 0 1 * *', async () => {
    ...
    try {
      ...
      const adminId = await db.getSetting('admin_chat_id');
      if (adminId) {
        await sendZaloMessage(adminId, `Hệ thống đã tự động dọn dẹp ${deletedCount} dữ liệu cũ từ trước ngày ${cutoffDate.toLocaleDateString('vi-VN')}.`);
      }
    } catch (err) {
