         ...
         adminReplyCell = `
           <div id="actionBox_${r.id}" style="display:flex; flex-direction:column; gap:8px;">
              ...
              <div style="display:flex; gap:6px; justify-content:flex-start;">
                  ...
                  <button onclick="rejectTicket(${r.id}, event)" style="padding:6px 16px; font-size:13px; background:#3b82f6; color:white; border:none; border-radius:9999px; cursor:pointer; white-space:nowrap; transition: all 0.2s; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">Hoàn tác</button>
              </div>
           </div>
         `;
     } else {
      ...
      <script>
        ...
        function showAlert(msg, isSuccess = false) {
          ...
          function rejectTicket(ticketId, event) {
              ...
              if (actionBox) {
                  const isRejecting = event && event.currentTarget && event.currentTarget.textContent.includes('Hoàn tác');
                  const placeholder = isRejecting ? "Lý do hoàn tác..." : "Lý do thay đổi trạng thái...";
